...
const { networkAuditFrontPage, webDisclaimer, tableOfContent_web, documentControl, introduction, scope, term_defination_legends, toolsAndReferences, executiveSummary, template_css, assessmentMethodology_network, tableOfContent_network, introduction_network, term_defination_legends_network, toolsAndReferences_network } = require('../json/auditJSON');
// import networkAuditFrontPage from "../json/auditJSON"
    ...
    function get_vulnerability_table2(data) {
        ...
        <div class="assessment_findings">
            ...
            <tr>
                ...
                <td class="content_table_white align-middle text-dark fw-bold ${data.Severity === "Informational" ? "bg-info" : data.Severity === "Critical" ? "bg-critical" : data.Severity === "High" ? "bg-danger" : data.Severity === "Medium" ? "bg-warning" : "bg-success"}">${data.Severity}</td>
                <td class="content_table_white align-middle fw-bold">Status</td>
                <td class="content_table_white align-middle fw-bold ${data.Status === "Open" ? "text-danger" : data.Status === "Closed" ? "text-success" : "text-dark"} " >${data.Status}</td>
            </tr>
            <tr>
...
`
    ...
    function get_vulnerability_table(data) {
        ...
        <h3 class="content_table_white vulnerability_dynamic_table_heading fw-bold" style="margin : 0px" id="">${get_vulberability_table_title_count()} ${data.Title}</h3>
                    ...
                    <tr>
                        <td class="content_table_white align-middle fw-bold" colspan="12" style="background-color:#E0E0E0;">Affected IP & Port  </td>
                    </tr>
...
`
    ...
    <!--FRont  page -->
    ${networkAuditFrontPage(reportData.ReportDetails.AuditeeOrganizationDetails.OrganizationLogo ,reportData.ReportDetails.AuditeeOrganizationDetails.OrganizationName)}

    <!-- page disclaimer -->
    ${webDisclaimer}

    <!-- page tableOfContent_web -->
    ${tableOfContent_network(reportData.ReportDetails.AuditDetails)}
    ...
    <!-- page introduction -->
    ${introduction_network(reportData.ReportDetails.ScopeDetails.ScopeName)}

    <!-- page scope -->
    ${scope(reportData.ReportDetails.ScopeDetails.ReportScope)}

    <!-- page term_defination_legends -->
    ${term_defination_legends_network}

    <!-- page assessmentMethodology -->
    ${assessmentMethodology_network}
