...
const { webAuditFrontPage, webDisclaimer, tableOfContent_web, documentControl, introduction, scope, term_defination_legends, toolsAndReferences, executiveSummary, template_css, assessmentMethodology_web_mobile_api } = require('../json/auditJSON');
// import webAuditFrontPage from "../json/auditJSON"


module.exports = (reportData, closure) => {
    // console.log("reportData", reportData.ReportDetails)
    ...
    var regexHTML = /(<([^>]+)>)/gi;
    // console.log("remediation", reportData.ReportDetails.AuditDetails[0].Remediation)
    ...
    function print_all_vulnerability_POC(data) {
        ...
        if (data === "null" || data === undefined) {
            ...
            for (let i = 0; i < data.length; i++) {
                // console.log("URL - ", `${process.env.AUDITOR_API_URI}/images/${data[i].POCFilename}`)
    ...
    function get_vulnerability_table2(data) {
        ...
        <div class="assessment_findings">
            <table class="table table-bordered mt-4" style="border-color:black;">

            <tbody>
            <tr  class="bg-sky-blue" >
                <td colspan="4" class="">
                <h3 class="content_table_white vulnerability_dynamic_table_heading fw-bold" style="margin : 0px" id=""> <span>${get_vulberability_table_title_count()} ${data.Title}</span></h3>
                </td>
            </tr>
            <tr>
                <td class="content_table_white align-middle fw-bold"  >Risk Level</td>
                <td style="min-width: 80%;"  class="content_table_white align-middle text-dark fw-normal ${data.Severity === "Informational" ? "bg-info" : data.Severity === "Critical" ? "bg-critical" : data.Severity === "High" ? "bg-danger" : data.Severity === "Medium" ? "bg-warning" : "bg-success"}">${data.Severity}</td>
...
`
    ...
    function get_vulnerability_table(data) {
        ...
        <h3 class="content_table_white vulnerability_dynamic_table_heading fw-bold" style="margin : 0px" id="">${get_vulberability_table_title_count()} ${data.Title}</h3>
                    ...
                    <tr>
                        <td colspan="3" class="content_table_white align-middle fw-bold"  style="background-color:#E0E0E0;">CVSS Risk Rating </td>
...
`
    ...
    <tbody>
        ...
        return `<tr>
           ...
           <td class="content_table_white  align-middle text-center fw-normal ${data.Severity === "Informational" ? "bg-info" : data.Severity === "Critical" ? "bg-critical" : data.Severity === "High" ? "bg-danger" : data.Severity === "Medium" ? "bg-warning" : "bg-success"} text-dark">${data.Severity}</td>
           <td class="content_table_white  fw-normal align-middle text-center ${data.Status === "Open" ? "text-danger" : data.Status === "Closed" ? "text-success" : "text-dark"} " >${data.Status}</td>
       </tr>`
    }).join('')}
    ...
    <div class="">
        <table class="table table-bordered mt-4" style="border-color:black;">
            <tbody>
                <tr  class="">
                    <td colspan="4"  style="background-color:#E0E0E0;" class="content_table_white align-middle fw-bold">Observation ID & Title </td>
                    <td colspan="8" class="p-2" >  <h3 class="content_table_white vulnerability_dynamic_table_heading fw-bold" style="margin : 0px" id="">8.1 ${reportData.ReportDetails.AuditDetails[0].Title}</h3>
                </td>
                </tr>
                <tr>
                    <td colspan="3" class="content_table_white align-middle fw-bold"  style="background-color:#E0E0E0;">CVSS Risk Rating </td>
    ...
    <!--FRont  page -->
    ${webAuditFrontPage(reportData.ReportDetails.AuditeeOrganizationDetails.OrganizationLogo , reportData.ReportDetails.AuditeeOrganizationDetails.OrganizationName)}
    ...
    ${documentControl(reportData.ReportDetails.DocumentControl,reportData.ReportDetails.ScopeDetails.ScopeName,  reportData.ReportDetails.AuditeeOrganizationDetails.OrganizationName, reportData.ReportDetails.AuditorOrganizationDetails.OrganizationName , closure)}

    <!-- page introduction -->
    ${introduction(reportData.ReportDetails.ScopeDetails.ScopeName , "Web")}
    ...
    ${assessmentMethodology_web_mobile_api("web apps")}
